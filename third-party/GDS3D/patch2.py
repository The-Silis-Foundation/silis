...
Renderer::init()
{
    ...
    //Build first VBO
    #ifdef GL_ARB_vertex_buffer_object
    if(enableVBO) glGenBuffersARB( 1, &instanceVBO );
#endif
    firstVBO = new VBO2_t;
...
Renderer::endRender()
{
    ...
    if(enableVBO && enableShaders) {
#ifdef GL_ARB_shader_objects
        glUseProgramObjectARB(shaderProgram);
        GLint locMat0 = glGetAttribLocationARB(shaderProgram, "instanceMat0");
        GLint locMat1 = glGetAttribLocationARB(shaderProgram, "instanceMat1");
        GLint locMat2 = glGetAttribLocationARB(shaderProgram, "instanceMat2");
        GLint locMat3 = glGetAttribLocationARB(shaderProgram, "instanceMat3");
        GLint locColor = glGetAttribLocationARB(shaderProgram, "instanceColor");
#endif
        ...
        for(int pass=0; pass<2; pass++) {
            ...
            for(size_t i=0; i<activeRecipes.size(); i++) {
                renderRecipe_t* recipe = activeRecipes[i];
                std::vector<instance_data_t>& insts = (pass == 0) ? recipe->instances : recipe->transparent_instances;
                if(insts.empty()) continue;
                
                // Bind geometry
                if(recipe->VBO != boundVBO) {
                ...
                glBindBufferARB(GL_ARRAY_BUFFER_ARB, instanceVBO);
                glBufferDataARB(GL_ARRAY_BUFFER_ARB, insts.size() * sizeof(instance_data_t), insts.data(), GL_DYNAMIC_DRAW_ARB);
                
                // Setup attributes
                if(locMat0 != -1) { glEnableVertexAttribArrayARB(locMat0); glVertexAttribPointerARB(locMat0, 4, GL_FLOAT, GL_FALSE, sizeof(instance_data_t), (void*)0); glVertexAttribDivisorARB(locMat0, 1); }
                if(locMat1 != -1) { glEnableVertexAttribArrayARB(locMat1); glVertexAttribPointerARB(locMat1, 4, GL_FLOAT, GL_FALSE, sizeof(instance_data_t), (void*)(4*sizeof(float))); glVertexAttribDivisorARB(locMat1, 1); }
                if(locMat2 != -1) { glEnableVertexAttribArrayARB(locMat2); glVertexAttribPointerARB(locMat2, 4, GL_FLOAT, GL_FALSE, sizeof(instance_data_t), (void*)(8*sizeof(float))); glVertexAttribDivisorARB(locMat2, 1); }
                if(locMat3 != -1) { glEnableVertexAttribArrayARB(locMat3); glVertexAttribPointerARB(locMat3, 4, GL_FLOAT, GL_FALSE, sizeof(instance_data_t), (void*)(12*sizeof(float))); glVertexAttribDivisorARB(locMat3, 1); }
                if(locColor != -1) { glEnableVertexAttribArrayARB(locColor); glVertexAttribPointerARB(locColor, 4, GL_FLOAT, GL_FALSE, sizeof(instance_data_t), (void*)(16*sizeof(float))); glVertexAttribDivisorARB(locColor, 1); }
                
                // Draw
                glDrawElementsInstancedARB(GL_TRIANGLES, recipe->numIndices, GL_UNSIGNED_SHORT, (char *) NULL+recipe->firstIndex*sizeof(GLushort), insts.size());
                total_tris += (recipe->numIndices / 3) * insts.size();
                
                // Disable attributes
                if(locMat0 != -1) { glVertexAttribDivisorARB(locMat0, 0); glDisableVertexAttribArrayARB(locMat0); }
                if(locMat1 != -1) { glVertexAttribDivisorARB(locMat1, 0); glDisableVertexAttribArrayARB(locMat1); }
                if(locMat2 != -1) { glVertexAttribDivisorARB(locMat2, 0); glDisableVertexAttribArrayARB(locMat2); }
                if(locMat3 != -1) { glVertexAttribDivisorARB(locMat3, 0); glDisableVertexAttribArrayARB(locMat3); }
                if(locColor != -1) { glVertexAttribDivisorARB(locColor, 0); glDisableVertexAttribArrayARB(locColor); }
            }
        }
#endif
    } else {
        ...
        for(int pass=0; pass<2; pass++) {
            ...
            for(size_t i=0; i<activeRecipes.size(); i++) {
                ...
                for(size_t j=0; j<insts.size(); j++) {
                    glLoadMatrixf((GLfloat*) &(insts[j].mat));
                    glColor4f(insts[j].color.GetX(), insts[j].color.GetY(), insts[j].color.GetZ(), insts[j].color.GetW());
                    if(enableVBO) {
                        if(recipe->VBO != boundVBO) {
                            glBindBufferARB( GL_ARRAY_BUFFER_ARB, recipe->VBO->vertbuffer );
                            glBindBufferARB( GL_ELEMENT_ARRAY_BUFFER_ARB, recipe->VBO->indexbuffer );
    ...
    for(size_t i=0; i<activeRecipes.size(); i++) {
        activeRecipes[i]->instances.clear();
        activeRecipes[i]->transparent_instances.clear();
    }
    activeRecipes.clear();

    // Empty Queue - just in case anything else used it, but we don't need it.
    queueLength = 0;
...
Renderer::renderObject(renderRecipe_t *recipe, MATRIX4X4 *mat, VECTOR4D *color, bool transparent)
{
    ...
    {
        if(recipe->instances.empty() && recipe->transparent_instances.empty()) {
            activeRecipes.push_back(recipe);
        }
        instance_data_t data;
        data.mat = *mat;
        data.color = *color;
        if(transparent) {
            recipe->transparent_instances.push_back(data);
        } else {
            recipe->instances.push_back(data);
        }
    }
