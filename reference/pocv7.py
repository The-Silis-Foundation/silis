...
class CodeEditor(tk.Frame):      #code editor ! nottttt gooood!!!
    """ (Unchanged) Editor with Line Numbers, Syntax Highlighting, Tab Fixes """
    ...
    def insert_tab_spaces(self, event):  #auto insert_tab_spaces: 4 tabs
    ...
    def update_line_numbers(self):
        ...
        self.gutter.yview_moveto(self.text.yview()[0])
    def highlight_syntax(self):       #syntax highlight
...
class ZoomableSchematic(tk.Frame):      #schematic viewer   
    """ UPDATED: Pan/Zoom with Keys """
    def __init__(self, parent):
        ...
        self.canvas.bind("<MouseWheel>", self.on_zoom) # Scroll Zoom
        
        # KEYBOARD BINDINGS (Must click canvas to focus first!)
        self.canvas.bind("<Button-1>", lambda e: self.canvas.focus_set())
        
        # Zoom Keys (+ / -)
        self.canvas.bind("<plus>", self.key_zoom_in)
        self.canvas.bind("<KP_Add>", self.key_zoom_in)
        self.canvas.bind("<minus>", self.key_zoom_out)
        self.canvas.bind("<KP_Subtract>", self.key_zoom_out)
        ...
        # Numpad Arrows (NumLock OFF usually sends these codes)
        self.canvas.bind("<KP_Left>", lambda e: self.canvas.xview_scroll(-1, "units"))
        self.canvas.bind("<KP_Right>", lambda e: self.canvas.xview_scroll(1, "units"))
        self.canvas.bind("<KP_Up>", lambda e: self.canvas.yview_scroll(-1, "units"))
        self.canvas.bind("<KP_Down>", lambda e: self.canvas.yview_scroll(1, "units"))
    ...
    def key_zoom_in(self, event):
        self.zoom_level *= 1.1; self.show_image()
...
class VerilogIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("WSL Verilog Workbench v8.1 (Fixed Syntax)")
        self.root.geometry("1400x900")
        self.root.config(cursor="arrow") 
        
        # Leader Key Defaults
        self.key_map = {"focus_editor":"c", "focus_terminal":"x", "focus_files":"v", "focus_schem":"z"}
        ...
        # 3. Schematic (Target of Arrow Pan / Zoom)
        ...
        # --- NEW: Right Arrow Autocomplete ---
        self.term_input.bind("<Right>", self.handle_arrow_autocomplete)
        ...
        self.log_system(f"Workbench v8.1 Ready. CWD: {self.cwd}")
        self.root.after(100, self.process_queue)

    # ================= SETTINGS =================
    def open_settings(self):
        win = tk.Toplevel(self.root); win.title("Settings"); win.geometry("300x250")
        tk.Label(win, text="Leader Key (` + ...)", font=("Arial", 10, "bold")).pack(pady=10)
        entries = {}
        ...
        def save():
            ...
            self.log_system("Keys updated."); win.destroy()
        tk.Button(win, text="Save", command=save, bg="#d4edda").pack(pady=15)

    def activate_leader_mode(self, event):
        self.leader_active = True; self.root.config(cursor="crosshair")
        self.root.after(1000, self.reset_leader); return "break"

    def reset_leader(self):
        self.leader_active = False; self.root.config(cursor="arrow")

    def handle_leader_followup(self, event):
        if not self.leader_active: return
        key = event.keysym.lower()
        if key == self.key_map["focus_editor"]: self.editor.focus_set(); self.log_system("Focus: Editor")
        elif key == self.key_map["focus_terminal"]: self.term_input.focus_set(); self.log_system("Focus: Terminal")
        elif key == self.key_map["focus_files"]: self.tree.focus_set(); self.log_system("Focus: Files")
        elif key == self.key_map["focus_schem"]: self.schematic_viewer.focus_set(); self.log_system("Focus: Schematic")
        self.reset_leader(); return "break"

    # ================= AUTOCOMPLETE (RIGHT ARROW) =================
    def handle_arrow_autocomplete(self, event):
        if self.term_mode != "SHELL": return
        
        # Get cursor pos and text
        full_text = self.term_input.get()
        cursor_pos = self.term_input.index(tk.INSERT)
        
        # Only autocomplete if cursor is at the end of the text
        if cursor_pos != len(full_text): return 

        # Find prefix (last word)
        if " " in full_text: base, prefix = full_text.rsplit(" ", 1); base += " "
        else: base, prefix = "", full_text
        
        if not prefix: return # Nothing to complete
        ...
        if len(matches) == 1:
            ...
            return "break" # Consume the Right Arrow so cursor doesn't move weirdly
        
        # If no match, let Right Arrow do its normal thing (move cursor)
        return
