...
for i, line in enumerate(lines):
    ...
    if in_layout:
        if 'col_x += max_w_in_col + 400' in line:
            new_lines.append(indent + line.lstrip().replace('400', 'channel_spacing_v'))
            continue
            
        if 'if len(blocks) <= 500:' in line and 'for h in h_segs:' in lines[i+1]:
            # This is the second `if len(blocks) <= 500:` which is for junctions.
            # Insert C router integration right before this!
            c_router_code = """
                if len(blocks) > 500:
                    import ctypes, os
            ...
            for cl in c_router_code.split('\n'):
                if cl.strip(): new_lines.append(indent + cl.strip() + '\n')
            new_lines.append(indent + line.lstrip())
            continue
