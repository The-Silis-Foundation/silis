...
class SiliconConfigHub(QDialog):
    ...
    def populate_pdks(self):
        ...
        for cfg in self.ide.pdk_mgr.configs:
            ...
            # Auto-select the currently active one
    ...
    def accept_config(self):
        ...
        r = self.pdk_table.currentRow()
        if r >= 0:
            self.ide.active_pdk = self.pdk_table.item(r, 0).data(Qt.ItemDataRole.UserRole)
        elif self.pdk_table.rowCount() > 0:
            self.ide.active_pdk = self.pdk_table.item(0, 0).data(Qt.ItemDataRole.UserRole)
            
        # 2. Grab Checked Macros
        checked_macros = []
...
from PyQt6.QtWidgets import QGraphicsItem # Make sure this is imported at the top
...
class MacroItem(QGraphicsRectItem):
    def __init__(self, inst_name, master_name, x, y, w, h, snap_w, snap_h, dbu):
        ...
        # Enable dragging and coordinate tracking
        self.setFlags(QGraphicsItem.GraphicsItemFlag.ItemIsSelectable |
                      QGraphicsItem.GraphicsItemFlag.ItemIsMovable |
                      QGraphicsItem.GraphicsItemFlag.ItemSendsGeometryChanges)
        
        # Pro EDA Styling
        self.setBrush(QBrush(QColor(80, 120, 200, 200))) # Solid Blueish
    ...
    def paint(self, painter, option, widget):
        super().paint(painter, option, widget)
        # Draw the Macro Name in the center
        painter.save()
        painter.setPen(Qt.GlobalColor.white)
        # Scale text up so it's readable relative to the massive DBU size
        font = QFont("Consolas", int(max(self.rect().width(), self.rect().height()) / 15))
        font.setBold(True)
        painter.setFont(font)
        # Need to flip text since CAD Y-axis is inverted
...
class SiliconPeeker(QGraphicsView):
    ...
    def enter_sandbox_mode(self, json_path, die_w_um=1000, die_h_um=1000):
        """Loads the macros from Init and creates an interactive floorplan."""
        if not os.path.exists(json_path):
            print("No sizes.json found. Skipping Sandbox.")
            return
        ...
        die_w_dbu = die_w_um * dbu
        die_h_dbu = die_h_um * dbu
        
        self.set_die_area(0, 0, die_w_dbu, die_h_dbu)
        
        # Spawn the Macros outside the Die for the user to drag in
        spawn_x = die_w_dbu + (50 * dbu)
        spawn_y = 50 * dbu
        
        self.macro_items = [] # Keep track of them
        for inst_name, info in data.get("macros", {}).items():
            m = MacroItem(inst_name, info["master"], spawn_x, spawn_y, info["w"], info["h"], snap_w, snap_h, dbu)
            self.scene.addItem(m)
            self.macro_items.append(m)
            spawn_y += info["h"] + (20 * dbu) # Stack them vertically
...
class BackendWidget(QWidget):
    def __init__(self, parent_ide):
        ...
        self.macro_mgr = MacroManager() # NEW
        self.active_pdk = None
        self.active_macros = [] # List of checked-out macros for this run
    ...
    def run_flow_step(self, step_name):
        ...
        if step_name == "Init":
            ...
            # The Magic TCL Dump Script (Patched for OpenDB API)
            tcl_content = f"""
...
# Default failsafe sizes
set site_w 460
set site_h 2720

# Search the loaded Libraries for the Site Grid dimensions
...
puts $out_file "{{"
puts $out_file "\\\"dbu\\\": $dbu,"
puts $out_file "\\\"site_w\\\": $site_w,"
puts $out_file "\\\"site_h\\\": $site_h,"
puts $out_file "\\\"macros\\\": {{"
...
foreach inst [$block getInsts] {{
    ...
    # Check if the cell is a MACRO/BLOCK (Not a standard logic gate)
    if {{[string match "BLOCK*" [$master getType]]}} {{
        ...
        puts -nonewline $out_file "\\\"$iname\\\": {{\\\"master\\\": \\\"$mname\\\", \\\"w\\\": $w, \\\"h\\\": $h}}"
        set first_inst 0
    }}
}}
puts $out_file "\\n}}\\n}}"
...
"""
        ...
        if step_name == "Floorplan": 
            # 1. Base Floorplan Command
            cmd = f"initialize_floorplan -die_area \"0 0 1000 1000\" -core_area \"10 10 990 990\" -site unithd;\n"
            
            # 2. Scrape Macro coordinates from SiliconPeeker
            if hasattr(self.peeker, 'macro_items'):
                for m in self.peeker.macro_items:
                    # Convert absolute DBU back to microns for OpenROAD
                    x_um = m.pos().x() / m.dbu
                    y_um = m.pos().y() / m.dbu
                    # Force placement and add a 10um protective routing halo
