...
class SiliconPeeker(QGraphicsView):
    ...
    def redraw(self):
        ...
        # 2. Heatmap (Background Layer)
        if self.show_heatmap:
            self.draw_heatmap(d)

        # 3. Nets (Flylines)
        if self.show_nets:
            pen = QPen(QColor(0, 0, 0, 40), 0) # Very faint black lines
        ...
        # 5. Pins & Labels
        if self.show_pins:
            font = QFont("Arial", 10) # Font size is in screen pixels, transform handles scale? 
            # Actually QGraphicsTextItem ignores view scale unless transformed.
            # We want scalable text.
            ...
            for rect, name in self.def_data.pins:
                ...
                text.setScale(rect.width() / 50) # Scale text relative to pin size
                text.setTransform(text.transform().scale(1, -1)) # Flip text upright
                text.setDefaultTextColor(QColor("black"))

        self.fitInView(self.scene.itemsBoundingRect(), Qt.AspectRatioMode.KeepAspectRatio)

    def draw_heatmap(self, die_rect):
        if die_rect.width() <= 0: return
        grid_sz = die_rect.width() / 20
        cols, rows = 20, 20
        counts = [[0]*cols for _ in range(rows)]
        
        for rect in self.def_data.comps_map.values():
            c = int((rect.x() - die_rect.x()) / grid_sz)
            r = int((rect.y() - die_rect.y()) / grid_sz)
        ...
        for r in range(rows):
            for c in range(cols):
                val = counts[r][c] / max_c if max_c > 0 else 0
                if val > 0:
                    g_rect = QRectF(die_rect.x() + c*grid_sz, die_rect.y() + r*grid_sz, grid_sz, grid_sz)
                    item = QGraphicsRectItem(g_rect)
                    item.setPen(Qt.PenStyle.NoPen)
                    # Green -> Red Gradient
                    color = QColor(int(255*val), int(255*(1-val)), 0, 100 + int(val*100))
