...
class SilisIDE(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Silis QT v3.3 (SV + Timeout)")
    ...
    def run_simulation(self):
        ...
        out = f"{base}.out"
        # iverilog handles -g2012 automatically mostly, but explicit is better if needed.
        # For now simple compile:
    ...
    def run_synthesis_flow(self):
        ...
        # ... (Rest of Sta/Tcl is same) ...
        # (Reusing existing Tcl generation)
    ...
    # --- SCHEMATIC GENERATION (FAILOVER SYSTEM) ---
    def generate_schematic(self):
        ...
        # ATTEMPT 1: VECTOR ENGINE
        use_netlist = (self.schem_engine == "NetlistSVG") or (self.schem_engine == "Auto" and shutil.which("netlistsvg"))
        if use_netlist:
            self.log_system("Attempting Vector Schematic...", "SYS")
            
            read_cmd = ""
            if src_v: read_cmd += f"read_verilog {' '.join(src_v)}; "
            if src_sv: read_cmd += f"read_verilog -sv {' '.join(src_sv)}; "
            
            cmd_json = f"yosys -p '{read_cmd} hierarchy -auto-top; proc; opt; write_json {base}.json'"
            subprocess.run(cmd_json, shell=True, cwd=root)
            
            if os.path.exists(os.path.join(root, f"{base}.json")):
                env = os.environ.copy()
                env["NODE_OPTIONS"] = "--max-old-space-size=4096" # Boost Memory
                
                try:
                    # TIMEOUT ADDED: 15 Seconds
                    res = subprocess.run(f"netlistsvg {base}.json -o {base}.svg", shell=True, cwd=root, env=env, timeout=15)
                    ...
                    if res.returncode == 0 and os.path.exists(os.path.join(root, f"{base}.svg")):
                     ...
                     self.log_system("Vector Engine Timed Out (Complexity). Fallback...", "WARN")
        
        # ATTEMPT 2: INDUSTRIAL FALLBACK (GRAPHVIZ)
        ...
        cmd_dot = f"yosys -p '{read_cmd} hierarchy -auto-top; proc; show -format dot -prefix {base}'"
