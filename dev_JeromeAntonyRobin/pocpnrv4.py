...
class BackendWidget(QWidget):
    ...
    def read_stdout(self):
        ...
        # --- NEW HANDSHAKE LOGIC ---
        # We wait for the version banner or prompt to know it's alive
            if self.pending_init and ("OpenROAD" in data or "openroad>" in data):
                ...
                self.pending_init = None # Clear the queue so we don't run it twice
    ...
    def run_flow_step(self, step_name):
        if step_name == "1. Init":
            ...
            try:
                ...
                self.pending_init = f"source {tcl_path}" # Queue the command
                self.term_log.append("[SYS] Restarting OpenROAD for fresh Init...")
                self.reset_backend() # Kill and Restart
                self.send_command_internal(f"source {tcl_path}")
            except Exception as e:
                self.term_log.append(f"[ERR] Failed to write init script: {e}")
            return

        # FIXED COMMANDS: Uses utilization 20 (low density) to act as a buffer.
        cmd_map = {
...
class SilisIDE(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Silis QT v4.7 (Fixed Floorplan Commands)")
