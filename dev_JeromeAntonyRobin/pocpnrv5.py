...
class DEFParser:
    def __init__(self, def_path):
        ...
        self.components = [] # List of (QRectF, type_str)
        self.pins = []       # List of QRectF
        self.dbu = 1000.0    
    ...
    def parse(self):
        ...
        # 3. Components (Now catches PLACED, FIXED, COVER)
        # Format: - instName model + STATUS ( x y ) ...
        # We also grab the 'model' to detect tapcells vs macros
        ...
        for model, x, y in comps:
            ctype = "TAP" if "tap" in model.lower() or "fill" in model.lower() else "STD"
            self.components.append((QRectF(int(x), int(y), std_w, std_h), ctype))

        # 4. Pins
        pins = re.findall(r'-\s+\S+\s+\+\s+NET\s+\S+.*\+\s+PLACED\s+\(\s+(-?\d+)\s+(-?\d+)\s+\)', content)
        pin_sz = 1 * self.dbu
        for x, y in pins:
            self.pins.append(QRectF(int(x), int(y), pin_sz, pin_sz))
...
class SiliconPeeker(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self.scale(1, -1) 
        self.def_data = None
        self.view_mode = "LAYOUT" # LAYOUT or HEATMAP

    def set_mode(self, mode):
        self.view_mode = mode
        if self.def_data: self.redraw()
    ...
    def redraw(self):
        ...
        if self.view_mode == "LAYOUT":
            # Draw Cells
            for rect, ctype in self.def_data.components:
                item = QGraphicsRectItem(rect)
                if ctype == "TAP":
                    item.setPen(Qt.PenStyle.NoPen)
                    item.setBrush(QBrush(QColor("#dddddd"))) # Grey Taps
                else:
                    item.setPen(QPen(QColor("#00509d"), 0))
                    item.setBrush(QBrush(QColor("#4cc9f0"))) # Blue Logic
                self.scene.addItem(item)
            
            # Draw Pins
            for rect in self.def_data.pins:
                item = QGraphicsRectItem(rect)
            ...
            if d.width() > 0 and d.height() > 0:
                ...
                for rect, _ in self.def_data.components:
                    c = int((rect.x() - d.x()) / grid_sz)
                    r = int((rect.y() - d.y()) / grid_sz)
                    if 0 <= c < cols and 0 <= r < rows:
                        counts[r][c] += 1
                
                max_c = max(max(row) for row in counts) if counts else 1
                if max_c == 0: max_c = 1
                ...
                for r in range(rows):
                    for c in range(cols):
                        ...
                        if val > 0:
                            alpha = int(val * 200) + 50
                            g_rect = QRectF(d.x() + c*grid_sz, d.y() + r*grid_sz, grid_sz, grid_sz)
                            item = QGraphicsRectItem(g_rect)
                            item.setPen(Qt.PenStyle.NoPen)
                            # Heatmap Gradient (Red = Dense, Green = Sparse)
                            color = QColor(255, int(255*(1-val)), 0, 150)
...
class BackendWidget(QWidget):
    def __init__(self, parent_ide):
        ...
        for step in self.steps:
            ...
            btn.setStyleSheet("""
                QPushButton { background: #e1e1e1; color: #333; border: 1px solid #aaa; border-radius: 4px; padding: 5px 10px; font-weight: bold; }
        ...
        btn_style.clicked.connect(self.toggle_view_style)
        r_lay.addWidget(btn_style)

        btn_gui = QPushButton("Show GUI"); btn_gui.clicked.connect(self.launch_native_gui)
        r_lay.addWidget(btn_gui)

        btn_rst = QPushButton("🔄 Reset"); btn_rst.setStyleSheet("color: red;")
        btn_rst.clicked.connect(self.reset_backend)
        r_lay.addWidget(btn_rst)
        
        cfg_btn = QPushButton("⚙ PDK"); cfg_btn.clicked.connect(self.open_pdk_selector)
    ...
    def toggle_view_style(self):
        mode = "HEATMAP" if self.peeker.view_mode == "LAYOUT" else "LAYOUT"
        self.peeker.set_mode(mode)
    ...
    def run_flow_step(self, step_name):
            ...
            # --- DYNAMIC COMMAND MAP ---
            # Note: Core Area is calculated relative to Die Area (Die - 10um margin)
            # Ideally, user edits this string directly in the popup.
            
            default_cmd = ""
            
            if step_name == "2. Floorplan":
                # Default to 400x400 die with 10um margins (10 10 390 390)
                default_cmd = f"initialize_floorplan -die_area \"0 0 400 400\" -core_area \"10 10 390 390\" -site unithd; {write_cmd}"
            
            elif step_name == "3. Tapcells":
                # Includes make_tracks to prevent pin errors
                default_cmd = f"tapcell -distance 14 -tapcell_master sky130_fd_sc_hd__tapvpwrvgnd_1; make_tracks; {write_cmd}"
                
            elif step_name == "4. IO Pins":
                default_cmd = f"place_pins -hor_layers met3 -ver_layers met4; {write_cmd}"
                
            elif step_name == "5. Place":
                default_cmd = f"global_placement -density 0.6; {write_cmd}"
                
            elif step_name == "6. CTS":
                default_cmd = f"clock_tree_synthesis -root_buf sky130_fd_sc_hd__clkbuf_4 -buf_list sky130_fd_sc_hd__clkbuf_4; {write_cmd}"
                
            elif step_name == "7. Route":
                default_cmd = f"detailed_route -output_drc reports/drc.rpt; {write_cmd}"
                
            elif step_name == "8. GDS":
                default_cmd = f"write_gds results/design.gds"

            # Prompt User (This is where you tweak the size!)
            ...
            if ok and text:
                ...
                QTimer.singleShot(1000, lambda: self.peeker.load_def_file(def_abs_path))
    ...
    def send_command_internal(self, cmd):
        ...
        if self.proc.state() == QProcess.ProcessState.Running: self.proc.write(f"{cmd}\n".encode())
        else: self.term_log.append(f"[MOCK] {cmd}")
        self.term_log.verticalScrollBar().setValue(self.term_log.verticalScrollBar().maximum())
