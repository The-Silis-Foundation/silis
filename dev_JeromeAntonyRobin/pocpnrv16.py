...
class ReportEngine:
    """Parses combined logs to extract metrics and generate pro-grade reports."""
...
╚══════╝ ╚═╝ ╚══════╝ ╚═╝ ╚══════╝
    ...
    def parse_metrics(full_log):
        metrics = {
            "area": "Unknown", "gates": "0", 
            "wns": "0.00", "freq": "Unknown",
            "status": "UNKNOWN", "critical_path": [], 
            "power_table": [], "errors": []
        }
        ...
        for i, line in enumerate(lines):
            ...
            if "Chip area for module" in line:
                ...
                if m: metrics["area"] = f"{float(m.group(1)):.2f}"
            
            if "sky130_fd_sc_hd__" in line:
                try: metrics["gates"] = str(int(metrics["gates"]) + 1)
                except: pass

            # --- 2. TIMING (OpenSTA) ---
            # Capture explicitly defined WNS or look for the violation line
            if "slack (VIOLATED)" in line:
                # Format: -3.32   slack (VIOLATED)
                parts = line.split()
                if parts: metrics["wns"] = parts[0]
                metrics["status"] = "VIOLATION"
            elif "wns" in line and metrics["wns"] == "0.00":
                # Fallback if specific violation line isn't found first
                m = re.search(r"([0-9.-]+)", line)
                if m: metrics["wns"] = f"{float(m.group(1)):.2f}"

            # --- 3. CRITICAL PATH ---
            if "Startpoint:" in line: 
                metrics["critical_path"] = [line] 
                in_path = True
            elif in_path:
                if "slack (" in line: # End of path report
                    metrics["critical_path"].append(line)
                    in_path = False
                elif any(x in line for x in ["Endpoint:", "Path Group:", "Path Type:"]):
                    metrics["critical_path"].append(line)
            
            # --- 4. POWER TABLE ---
            if "Group" in line and "Internal" in line and "Switching" in line:
                in_power = True
                metrics["power_table"] = [] # Reset
                # Grab header
                metrics["power_table"].append(line) 
                continue
            ...
            if in_power:
                if "----------------" in line:
                    ...
                    # Empty line usually ends the block in STA logs
                    in_power = False
                else:
                    metrics["power_table"].append(line)
                    # If we just added the percentages line (starts with digits and %), stop
                    if re.match(r"^\d+\.\d+%", line): in_power = False
        ...
        # Final Status Check
        if metrics["status"] == "UNKNOWN" and float(metrics["wns"]) < 0:
            metrics["status"] = "VIOLATION"
        elif metrics["status"] == "UNKNOWN" and float(metrics["wns"]) >= 0:
            metrics["status"] = "TIMING MET"
    ...
    def generate_report(metrics):
        ...
        # Format Power Table for the specific "Nicer ASCII Spacing" request
        # We assume the raw log has tabs/spaces, we force alignment if needed, 
        # but usually simply indenting the block looks best.
        pt_str = "\n".join(metrics["power_table"]) if metrics["power_table"] else "No Power Analysis Data Available."
        
        content = f"""{ReportEngine.BRANDING}
Silis -- Silicon Scaffold
Organization: The Silis Foundation
___________________________________________

[ SILICON BUILD SUMMARY ]
--------------------------------------------------
Date:       {now}
Target:     Skywater 130nm (High Density)
Status:     {metrics['status']}

[ PHYSICAL METRICS ]
--------------------------------------------------
Total Area:     {metrics['area']} um^2
Gate Count:     {metrics['gates']} instances

[ TIMING METRICS ]
--------------------------------------------------
WNS (Slack):    {metrics['wns']} ns
Status:         {metrics['status']}

[ CRITICAL PATH INFO ]
--------------------------------------------------
{chr(10).join(metrics['critical_path']) if metrics['critical_path'] else "No Timing Paths Found"}
...
[ ISSUES & ERRORS ]
--------------------------------------------------
{chr(10).join(metrics['errors']) if metrics['errors'] else "No Critical Errors Found."}

___________________________________________
Engine Credits:
Synthesis: Yosys Open SYnthesis Suite
Timing:    OpenSTA
Generated by Silis v2.1
"""
        return content
...
class SynthesisTab(QWidget):
    def __init__(self, ide):
        ...
        self.list_err.setStyleSheet("background:#fff; color:#cf222e; font-family:Consolas; border:1px solid #d0d7de;")
        ...
        right_col.setFixedWidth(340) # Slightly wider for Power Table
        r_lay = QVBoxLayout(right_col)
        
        # 1. Status Card
        self.card_status = QLabel("READY")
        self.card_status.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.card_status.setStyleSheet("background:#eaeef2; color:#57606a; font-size:16px; font-weight:bold; padding:15px; border-radius:6px; border: 1px solid #d0d7de;")
        r_lay.addWidget(self.card_status)
        
        # 2. Metrics Grid
        grid_w = QWidget()
        grid = QGridLayout(grid_w)
        l_style = "color: #57606a;"
        v_style = "font-weight:bold; font-size:14px; color: #24292f;"
        ...
        for i in range(grid.count()): 
            ...
            if isinstance(w, QLabel) and w.text().endswith(":"): w.setStyleSheet(l_style)
            elif isinstance(w, QLabel) and "--" in w.text(): w.setAlignment(Qt.AlignmentFlag.AlignRight)
        r_lay.addWidget(grid_w)
        
        # 3. Critical Path
        r_lay.addWidget(QLabel("<b style='color:#24292f'>Critical Path Trace:</b>"))
        self.path_view = QTextEdit(); self.path_view.setReadOnly(True)
        self.path_view.setMaximumHeight(80)
        self.path_view.setStyleSheet("background:#ffffff; color:#24292f; font-size:10px; border:1px solid #d0d7de;")
        r_lay.addWidget(self.path_view)
        ...
        self.preview.setStyleSheet("font-family:Consolas; font-size:9px; background:#ffffff; color:#57606a; border:1px solid #d0d7de;")
        r_lay.addWidget(self.preview)
        
        btn_save = QPushButton("Save Report"); btn_save.setStyleSheet(btn_style)
    ...
    def update_dashboard(self):
        ...
        if m["status"] == "TIMING MET":
            self.card_status.setText("TIMING MET")
            self.card_status.setStyleSheet("background:#2da44e; color:white; font-size:16px; font-weight:bold; padding:15px; border-radius:6px;")
        elif m["status"] == "VIOLATION":
            self.card_status.setText("TIMING FAIL")
            self.card_status.setStyleSheet("background:#cf222e; color:white; font-size:16px; font-weight:bold; padding:15px; border-radius:6px;")
        else:
            self.card_status.setText("UNKNOWN")
            self.card_status.setStyleSheet("background:#eaeef2; color:#57606a; font-size:16px; font-weight:bold; padding:15px; border-radius:6px; border: 1px solid #d0d7de;")
            
        self.val_wns.setText(f"{m['wns']} ns")
        color = "#cf222e" if float(m['wns']) < 0 else "#2da44e"
        self.val_wns.setStyleSheet(f"font-weight:bold; font-size:14px; color: {color};")
        self.val_area.setText(m['area'])
        self.val_gates.setText(m['gates'])
        
        self.path_view.setText("\n".join(m['critical_path']))
        self.list_err.clear()
        for e in m['errors']: self.list_err.addItem(e)
        if m['errors']: self.log_tabs.setCurrentIndex(1)
        
        rpt = ReportEngine.generate_report(m)
    ...
    def save_report(self):
        ...
        # 1. Intelligence: Find the true Project Root
        # If we are currently inside 'source/', go up one level (..)
        current_dir = self.ide.cwd
        if os.path.basename(current_dir) == "source":
             project_root = os.path.dirname(current_dir)
        else:
             project_root = current_dir
             
        # 2. Ensure 'reports' folder exists in the PROJECT ROOT
        ...
        default_name = f"silis_report_{timestamp}.txt"
        default_path = os.path.join(report_dir, default_name)
        
        # 4. Open Dialog pre-filled with the correct ../reports path
        ...
        if path:
            ...
            self.ide.log_system(f"Report saved to: {os.path.basename(path)}")
