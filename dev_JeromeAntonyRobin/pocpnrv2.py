...
class PDKManager:
    """
    Manages scanned and manual PDK configurations.
    """
    def __init__(self):
    ...
    def add_manual_config(self, name, tlef, lef, lib):
        ...
        # Remove existing manual entry with same name if exists
        self.configs = [c for c in self.configs if c['name'] != name]
        self.configs.insert(0, entry) # Add to top
        self.save_cache()
        return entry

    def scan_root(self, root_path):
        """
        Heuristic scanner for SkyWater/OpenLane style directories.
        """
        found = []
        ...
        # Merge manual configs (top priority) with scanned
...
class PDKSelector(QDialog):
    ...
    def populate(self):
        ...
        for i, cfg in enumerate(self.mgr.configs):
    ...
    def trigger_manual(self):
        ...
        if d.exec() == QDialog.DialogCode.Accepted:
            ...
            new_entry = self.mgr.add_manual_config(name, tlef, lef, lib)
            
            # CRITICAL FIX: Clear search so the new entry shows up
            self.search.clear()
            self.populate()
            
            # Auto-select the newly created item (it's at index 0)
            self.table.selectRow(0)
            self.table.setFocus()
...
class BackendWidget(QWidget):
    def __init__(self, parent_ide):
        ...
        if shutil.which("openroad"):
            ...
            self.term_log.append("[SYS] OpenROAD backend started.")
        else:
    ...
    def open_pdk_selector(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            self.term_log.append(f"   Tech: {os.path.basename(self.active_pdk['tlef'])}")
            self.term_log.append(f"   Macro: {os.path.basename(self.active_pdk['lef'])}")
            return True
        return False
    ...
    def run_flow_step(self, step_name):
        if step_name == "1. Init":
            ...
            if not os.path.exists(netlist_path):
                ...
                self.term_log.append("[WARN] Netlist not found! Using source Verilog.")
        ...
        cmd_map = {
            "2. Floorplan": "initialize_floorplan -utilization 50 -aspect_ratio 1 -core_space 2 -site unithd",
...
class SilisIDE(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Silis QT v4.5 (Nuclear PDK Override)")
