...
class SilisIDE(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Silis QT v2.9 (XML Engine)")
    ...
    # --- SCHEMATIC GENERATION (XML ENGINE) ---
    def generate_schematic(self):
        ...
        if not synth_files: self.log_system("No design files found.", "ERROR"); return

        self.log_system(f"Schematic ({self.schem_engine})...", "SYS")
        use_netlist = (self.schem_engine == "NetlistSVG") or (self.schem_engine == "Auto" and shutil.which("netlistsvg"))

        if use_netlist:
            cmd = f"yosys -p 'read_verilog {' '.join(synth_files)}; hierarchy -auto-top; proc; opt; write_json {base}.json'"
            subprocess.run(cmd, shell=True, cwd=root)
            if os.path.exists(os.path.join(root, f"{base}.json")):
                subprocess.run(f"netlistsvg {base}.json -o {base}.svg", shell=True, cwd=root)
                self.patch_netlist_svg(os.path.join(root, f"{base}.svg")) # SAFE XML
                self.schematic.load_schematic(os.path.join(root, f"{base}.svg"))
            else: self.log_system("Yosys JSON Gen Failed", "ERROR")
        else:
            # Fallback
            cmd = f"yosys -p 'read_verilog {' '.join(synth_files)}; hierarchy -auto-top; proc; show -format dot -prefix {base}'"
            subprocess.run(cmd, shell=True, cwd=root)
            if os.path.exists(os.path.join(root, f"{base}.dot")):
                subprocess.run(f"dot -Tsvg {base}.dot -o {base}.svg", shell=True, cwd=root)
                self.schematic.load_schematic(os.path.join(root, f"{base}.svg"))

    def patch_netlist_svg(self, path):
        """
        SAFE XML PARSER: Replaces attributes using ElementTree
        to ensure valid SVG structure while enforcing High Contrast.
        """
        if not os.path.exists(path): return
        try:
            # Register namespaces often found in SVG
            ...
            # Namespace map for finding tags
            ...
            for elem in root.iter():
                ...
                # 1. PATHS (Wires): Force Black Stroke, No Fill
                if tag in ['path', 'line', 'polyline']:
                    elem.attrib['stroke'] = 'black'
                    elem.attrib['stroke-width'] = '2'
                    elem.attrib['fill'] = 'none'
                    # Remove style if present to avoid conflicts
                    if 'style' in elem.attrib: del elem.attrib['style']

                # 2. TEXT: Force Black Fill
                elif tag == 'text':
                    elem.attrib['fill'] = 'black'
                    elem.attrib['stroke'] = 'none'
                    if 'style' in elem.attrib: del elem.attrib['style']

                # 3. SHAPES (Gates/Cells): Black Stroke
                elif tag in ['rect', 'circle', 'polygon', 'ellipse']:
                    # Only stroke logic gates, don't fill them black
                    elem.attrib['stroke'] = 'black'
                    elem.attrib['stroke-width'] = '2'
                    # Keep existing fill if it's not white/none, otherwise ensure visibility
                    # Usually better to leave fill alone for gates, just ensure outline
                    if 'style' in elem.attrib: del elem.attrib['style']
                
                # 4. GROUPS (g): Sometimes carry styles
                elif tag == 'g':
                    if 'style' in elem.attrib: del elem.attrib['style']

            tree.write(path)
        except Exception as e:
            print(f"XML Patch Error: {e}") # Fallback to unpatched
