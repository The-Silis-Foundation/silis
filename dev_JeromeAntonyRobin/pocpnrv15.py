...
class ReportEngine:
    """Parses combined logs to extract metrics."""
...
╚══════╝ ╚═╝ ╚══════╝ ╚═╝ ╚══════╝
    ...
    def parse_metrics(full_log):
        metrics = {
            ...
            "status": "UNKNOWN", "critical_path": [], "errors": []
        }
        ...
        for line in lines:
            # 1. Physical Metrics (Yosys)
            if "Chip area for module" in line:
                m = re.search(r"Chip area for module.*:\s+([0-9.]+)", line)
                if m: metrics["area"] = f"{float(m.group(1)):.2f}"
            
            # 2. Gate Count (Heuristic)
            if "sky130_fd_sc_hd__" in line:
                try:
                    current_gates = int(metrics["gates"])
                    metrics["gates"] = str(current_gates + 1)
                except: pass

            # 3. Timing Metrics (OpenSTA)
            if "wns" in line or "worst slack" in line:
                m = re.search(r"([0-9.-]+)", line)
                if m: 
                    val = float(m.group(1))
                    metrics["wns"] = f"{val:.2f}"
                    metrics["status"] = "TIMING MET" if val >= 0 else "VIOLATION"

            # 4. Critical Path Parsing
            if "Startpoint:" in line: 
                metrics["critical_path"] = [line.strip()] # Reset path
                in_path = True
            elif in_path and ("Endpoint:" in line or "Path Group:" in line or "Path Type:" in line):
                metrics["critical_path"].append(line.strip())
            elif in_path and "slack" in line and "VIOLATED" in line:
                metrics["critical_path"].append(f"FINAL SLACK: {line.strip()}")
...
class SynthesisTab(QWidget):
    def __init__(self, ide):
        ...
        self.path_view.setMaximumHeight(100)
    ...
    def save_report(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Save Report", self.ide.cwd, "Text Files (*.txt)")
        if path:
