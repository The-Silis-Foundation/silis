...
# ================= ROBUST STATE-MACHINE DEF PARSER =================
    ...
    def parse(self):
        try:
            with open(self.path, 'r') as f:
                # LINE BY LINE STREAM PARSING (Low Memory, High Robustness)
                lines = f.readlines()
            ...
            for line in lines:
                ...
                # 3. COMPONENTS (State Machine)
                    ...
                    # State 1: Start of Component definition
                    if line.startswith("-"):
                        if len(parts) >= 3:
                            current_comp_name = parts[1]
                            current_comp_model = parts[2]
                        else:
                            current_comp_name = None # Reset if malformed

                    # State 2: Look for placement in this line OR subsequent lines
                    if current_comp_name:
                        # Look for keyword PLACED or FIXED or COVER followed by ( x y )
                        if "PLACED" in line or "FIXED" in line or "COVER" in line:
                            if "(" in parts:
                                try:
                                    ...
                                    current_comp_name = None # Done with this comp
                                except: pass
                ...
                # 4. PINS
                    ...
                    if line.startswith("-"):
                        ...
                        if len(parts) > 2:
                            name = parts[1]
                            # Try simple single-line match first (most common for pins)
                            try:
                                if "(" in parts:
                                    idx = parts.index("(")
                                    x = int(parts[idx+1])
...
class SiliconPeeker(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self.scale(1, -1) # EDA Coordinates (Y-flip)
    ...
    def redraw(self):
        try:
            ...
            # Heatmap = White background (user request), Layout = White
            die.setBrush(QBrush(QColor("#FFFFFF")))
            die.setZValue(-10)
            self.scene.addItem(die)

            if self.def_data.component_count == 0 and d.width() > 0:
                t = self.scene.addText("No Components / Parsing Error")
            ...
            if self.show_heatmap:
                ...
                if self.show_power:
                    for r in self.def_data.power_rails:
                        ...
                        item.setBrush(QBrush(QColor("#ffaa00"))) # Orange rails
                        item.setZValue(-5)
                ...
                if self.show_insts:
                    ...
                    for name, rect in self.def_data.comps_map.items():
                        count += 1
                        if count > 30000: break # Perf limit
            ...
            # PINS (Triangles) - Always Visible
            if self.show_pins:
                for rect, name in self.def_data.pins:
                    cx, cy = rect.center().x(), rect.center().y()
                    sz = max(50 * self.def_data.dbu, rect.width() * 2) # Min size or scaled
    ...
    def draw_organic_heatmap(self, die_rect):
        ...
        expansion = 25 * self.def_data.dbu 
        # Red with alpha 15/255 -> stacks up to dark red
        color = QColor(255, 0, 0, 15) 
        brush = QBrush(color)
...
class SilisIDE(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Silis QT v5.2 (Unified Interface)")
