...
class SchematicWorker(QThread):
    ...
    def run(self):
        ...
        self.log.emit("Generating Vector Deck (Background)...", "SYS")
        ...
        if use_netlist:
            ...
            if os.path.exists(json_path):
                ...
                try:
                    subprocess.run(f"netlistsvg {self.base}.json -o {self.base}.svg", 
        ...
        self.log.emit("Switching to Industrial View (Graphviz)...", "WARN")
        ...
        self.log.emit("Generation Failed. Check syntax/Top Module.", "ERROR")

    def finalize(self, svg_path):
        # Patch SVG for visibility
        try:
            ...
            for e in root.iter():
                ...
                if tag in ['path','line','polyline']: 
                    e.attrib['stroke']='black'; e.attrib['fill']='none'; 
                    if 'style' in e.attrib: del e.attrib['style']
                elif tag == 'text': 
                    e.attrib['fill']='black'; e.attrib['stroke']='none'; 
                    if 'style' in e.attrib: del e.attrib['style']
            tree.write(svg_path)
        except: pass
        
        # Inject into External Template
        template_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "vector_deck_template.html")
        
        if not os.path.exists(template_path):
            self.log.emit("Template missing! Creating default...", "WARN")
            with open(template_path, 'w') as f: 
                f.write("<html><body>{{SVG_CONTENT}}</body></html>")

        with open(svg_path, 'r') as f: svg_content = f.read()
        with open(template_path, 'r') as f: html_template = f.read()
        
        html_content = html_template.replace("{{SVG_CONTENT}}", svg_content)
...
class SilisSchematic(QWidget):
    """
    LAUNCHER WIDGET: Includes WSL Path Fix
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.layout = QVBoxLayout(self)
        self.status = QLabel("Vector Deck Standby")
        self.status.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.status.setStyleSheet("color: gray; font-size: 14px; font-weight: bold;")
        
        self.btn_launch = QPushButton("Launch Vector Deck ↗")
        self.btn_launch.setStyleSheet("font-size: 16px; padding: 15px; font-weight: bold; background: #007ACC; color: white; border-radius: 5px;")
        self.btn_launch.clicked.connect(self.launch_last)
    ...
    def set_ready(self, path):
        self.last_path = path
        self.btn_launch.setEnabled(True)
        self.status.setText("Schematic Ready")
        self.status.setStyleSheet("color: #00AA00; font-size: 16px; font-weight: bold;")
        self.launch_last() 

    def launch_last(self):
        if not self.last_path: return
        
        abs_path = os.path.abspath(self.last_path)
        
        # --- WSL PATH FIX ---
        if "microsoft" in os.uname().release.lower():
            try:
                # 1. Translate Linux path to Windows path (e.g. \\wsl$\...)
                result = subprocess.run(["wslpath", "-w", abs_path], capture_output=True, text=True)
                if result.returncode == 0:
                    win_path = result.stdout.strip()
                    # 2. Launch explorer with the WINDOWS path
                    subprocess.run(["explorer.exe", win_path])
                    return
            except Exception as e:
                print(f"WSL Translation failed: {e}")

        # --- LINUX FIX ---
        browsers = ['google-chrome', 'google-chrome-stable', 'chromium', 'firefox', 'microsoft-edge']
        for b in browsers:
            if shutil.which(b):
                try:
                    subprocess.Popen([b, abs_path])
                    return
                except: pass
...
class SilisIDE(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Silis QT v3.9 (Vector Deck Edition)")
    ...
    def eventFilter(self, source, event):
        if event.type() == QEvent.Type.KeyPress:
            ...
            if self.sk_active:
                ...
                # elif txt == self.key_map["focus_schem"]: self.schematic.setFocus() # Removed
    ...
    # --- SCHEMATIC GENERATION (WORKER + LAUNCHER) ---
    def generate_schematic(self):
        ...
        self.schematic.status.setText("Processing...")
        self.schematic.status.setStyleSheet("color: orange; font-weight: bold;")
        ...
        self.worker.finished.connect(self.schematic.set_ready)
        self.worker.start()
