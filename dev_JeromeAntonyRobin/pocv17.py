...
class SilisIDE:
    def __init__(self, root):
        self.root = root
        self.root.title("Silis v35 (The Librarian)")
        ...
        self.log_system(f"Silis v35 Ready. CWD: {self.cwd}", "SYS")
    ...
    def run_synthesis_flow(self):
        ...
        self.log_system(f"Starting Synthesis for: {target_base}", "SYS")
        work_dir = self.ensure_project_workspace(target_base)
        sdc_data = self.open_constraint_wizard(target_base)
        if not sdc_data: 
            self.log_system("Synthesis Cancelled.", "WARN"); return
            
        with open(os.path.join(work_dir, "netlist", f"{target_base}.sdc"), 'w') as f: f.write(sdc_data)
        ...
        self.log_system("Tech Mapping...", "SYS")
        try:
            ...
            if res.returncode == 0:
                self.log_system("Synthesis Successful!", "SUCCESS")
                ...
                if area_match:
                    ...
                    self.log_system(f"Estimated Area: {area}", "SYS")
            else: self.log_system("Synthesis Failed.", "ERROR")
        except Exception as e: self.log_system(f"Synth Exception: {e}", "ERROR")
    ...
    def run_simulation(self):
        ...
        # Check if current file is the TB (and was just moved to source/)
        ...
        temp_tb = "._temp_tb.v"
        try:
            # FIX: Force read from src_dir
            read_path = os.path.join(src_dir, tb_file)
            if not os.path.exists(read_path): 
                 self.log_system(f"Error: TB {tb_file} not found in source/", "ERROR"); return
            ...
            # Write temp TB to Work Dir (Root of project)
            ...
            if not compile_list:
                self.log_system("Error: No sources found for compile.", "ERROR"); return
